...
DAG_ID="flexion_patients_etl_dag"
...
def _load_config(ti, gcs_bucket,gcs_path):
    ...
    ti.xcom_push(key="patients_gcs_bucket", value=data["patients_gcs_bucket"])
    ti.xcom_push(key="patients_data_gcs_path", value=data["patients_data_gcs_path"])
    ti.xcom_push(key="rerun_flag", value=data["rerun_flag"])
    ti.xcom_push(key="rerun_dt", value=data["rerun_dt"])
    ti.xcom_push(key="patients_bq_dataset_stg", value=data["patients_bq_dataset_stg"])
    ti.xcom_push(key="patients_bq_table_stg", value=data["patients_bq_table_stg"])
    ti.xcom_push(key="patients_bq_dataset", value=data["patients_bq_dataset"])
    ti.xcom_push(key="patients_bq_table", value=data["patients_bq_table"])
    ti.xcom_push(key="insert_patients_query_path", value=data["insert_patients_query_path"])
    _read_query(ti,gcs_bucket,data["insert_patients_query_path"],"insert_patients_query")
    _read_query(ti,gcs_bucket,data["merge_patients_query_path"],"merge_patients_query")
    return data
...
with models.DAG(
    ...
    read_config = PythonOperator(
        ...
        op_kwargs={"gcs_bucket":CONFIG_GCS_BUCKET, "gcs_path":patients_CONFIG_GCS_PATH},
    )
    ...
    drop_stg_table = BigQueryExecuteQueryOperator(
        ...
        sql="drop table if exists "+str("{{ ti.xcom_pull(task_ids='read_config_file', key = 'patients_bq_dataset_stg' ) }}")
            +"."+str("{{ ti.xcom_pull(task_ids='read_config_file', key = 'patients_bq_table_stg' ) }}"),
        use_legacy_sql=False,
   )

    load_stg_table = GCSToBigQueryOperator(
        task_id = 'load_stg_table',
        bucket = str("{{ ti.xcom_pull(task_ids='read_config_file', key = 'patients_gcs_bucket' ) }}"),
        source_objects = str("{{ ti.xcom_pull(task_ids='read_config_file', key = 'patients_data_gcs_path' ) }}")+"/"+rerun_dt+"/*.csv",
        destination_project_dataset_table = str("{{ ti.xcom_pull(task_ids='read_config_file', key = 'patients_bq_dataset_stg' ) }}")+"."
                                                +str("{{ ti.xcom_pull(task_ids='read_config_file', key = 'patients_bq_table_stg' ) }}"),
        source_format = "CSV",
        skip_leading_rows = 1,
        field_delimiter = ",",
    ...
    branch_op = BranchPythonOperator(
        ...
        op_kwargs={"bq_dataset": str("{{ ti.xcom_pull(task_ids='read_config_file', key = 'patients_bq_dataset' ) }}"),
                "bq_table":str("{{ ti.xcom_pull(task_ids='read_config_file', key = 'patients_bq_table' ) }}")},
        dag=dag)


    merge_task = BigQueryExecuteQueryOperator(
        task_id="merge_task",
        sql=str("{{ ti.xcom_pull(task_ids='read_config_file', key = 'merge_patients_query' ) }}"),
        use_legacy_sql=False,
   )
    
    insert_task = BigQueryExecuteQueryOperator(
        task_id="insert_task",
        sql=str("{{ ti.xcom_pull(task_ids='read_config_file', key = 'insert_patients_query' ) }}"),
        use_legacy_sql=False,
   )
